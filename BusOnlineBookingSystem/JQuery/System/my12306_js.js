...
if (!window.dhtmlx) {
    ...
    b(document).ready(function () {
        ...
        if (user_name && user_regard) {
            ...
            if (notify_TWO_2 && "" != notify_TWO_2) {
                if (notify_TWO_2 != '完成手机双向核验，即可使用手机号码直接登录的新服务，解除您遗忘用户名的烦恼。') {
                    dhtmlx.alert({
                        title: '温馨提示',
                        ok: '确定',
                        text: notify_TWO_2,
        ...
        b("#link_for_changePassword").on("click",
        function () {
            otsRedirect("post", ctx + "userSecurity/loginPwd", {
                req_flag: "init"
            })
        });
        b("#link_for_ticket").on("click",
        function () {
            otsRedirect("post", ctx + "leftTicket/init")
        });
        b("#link_for_reSendEmail").on("click",
        function () {
            b.ajax({
        ...
        b("#link_for_infor").on("click",
        function () {
            otsRedirect("post", ctx + "modifyUser/initQueryUserInfo")
        });
        b("#link_for_edit").on("click",
        function () {
            otsRedirect("post", ctx + "modifyUser/initQueryUserInfo")
        });
        b.sidebar_init(-1);
